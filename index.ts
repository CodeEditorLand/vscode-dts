			...
			.then(async () => {
				if (version) {
					...
					if (versionMatch !== version) {
						console.log(
							toRedString(
								`Version mismatch for ${name}: Latest is ${versionMatch}, the request version ${version} DOES NOT exist.`,
							),
						);
					}
				}
...
function handleDefaultDownload(gitTagOrBranch: string, force?: boolean) {
	...
	download(url, outPath)
		.catch(() => download(legacyUrl, outPath))
		.then(() => {
			if (force) {
...
function removeNodeModulesTypes() {
	if (fs.existsSync("node_modules/vscode/vscode.d.ts")) {
		prompts({
			...
			name: "value",
			message:
				"Remove conflicting vscode typing at node_modules/vscode/vscode.d.ts?",
		}).then((res) => {
			if (res.value) {
				fs.unlinkSync("node_modules/vscode/vscode.d.ts");
				console.log("Removed node_modules/vscode/vscode.d.ts");
		...
		prompts({
			...
			name: "value",
			message:
				"Remove conflicting vscode typing at node_modules/@types/vscode?",
		}).then((res) => {
			if (res.value) {
				fs.rm(
					"node_modules/@types/vscode",
